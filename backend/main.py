...
from services.devil_advocate import critique_plan
...
@app.post("/api/critique-plan")
async def critique_plan_endpoint(request: CritiquePlanRequest):
    if not request.question.strip():
        raise HTTPException(status_code=400, detail="Question cannot be empty")
    try:
        result = await asyncio.to_thread(critique_plan, request.question, request.plan)
        return result
    except Exception as exc:
        msg = str(exc)
        if "credit balance is too low" in msg or "billing" in msg.lower():
